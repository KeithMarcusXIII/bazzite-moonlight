...
The `remove:` property is a list of Flatpak IDs to uninstall on every boot. It runs after the `install:` step and follows the same per-configuration scope. If omitted, the default is an empty list — no flatpaks will be removed.

The remove list is validated against Flathub at build time (same as the install list), catching typos or unavailable IDs early. At runtime, `flatpak remove --noninteractive` silently skips any non-installed packages, so the remove list is safe to use even if some IDs are not present on the system.
