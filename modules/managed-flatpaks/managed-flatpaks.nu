...
const usrSharePath = "/usr/share/bluebuild/managed-flatpaks"
const libExecPath = "/usr/libexec/bluebuild/managed-flatpaks"
...
def main [configStr: string] {
    ...
    if ('user' in $config or 'system' in $config) {
        ...
        print $"(ansi yellow_reverse)HINT(ansi reset): the managed-flatpaks module has been updated with breaking changes!"
        print $"It seems like you are trying to run the new (ansi default_italic)managed-flatpaks(ansi reset) module with configuration made for the older default-flatpaks format."
        print $"You can read the docs to migrate, or switch back to the old module like this (ansi default_italic)type: default-flatpaks@v1(ansi reset)"
        exit 1
    }
