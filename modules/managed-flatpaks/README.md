...
:::note
If you are migrating from the `default-flatpaks` module, your existing v3 configuration is compatible — just change `type: default-flatpaks` to `type: managed-flatpaks`.
:::

The `managed-flatpaks` module can be used to install Flatpaks from a configurable remote on every boot. By default the module will remove the Fedora Flatpak remote and install the Flathub remote, but you can also configure it to install other Flatpaks from other remotes.
...
The `managed-flatpaks` module configuration is based on a list of `configurations:` that each set a scope to install in (`system` or `user`), a Flatpak repository to set up, and a list of Flatpaks to install from the repository.
...
The `remove:` property is a list of the Flatpak IDs to uninstall. If omitted, no Flatpaks will be removed. This is useful for removing pre-installed Flatpaks that are not wanted in your custom image. Unlike `install:`, the `remove:` list is not validated against Flathub — any Flatpak ID can be specified, and `flatpak remove --noninteractive` will silently skip IDs that are not installed.
